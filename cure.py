#!/usr/bin/env python
'''
    This file is an implementation of Institute of Biblio-Immunology's First
    Communique (see https://pastebin.com/raw/E1xgCUmb)

    It removes the various manifestations of BooXtream's 'Social DRM' from ePub
    files.

    The seven DRM trackers that are removed are as such:

    "WM0-2 are overt (readily visible) watermarks and are optional (meaning
    they may not necessarily be present):
...
def deterministicNameGen():
    string = '''We need to take information, wherever it is stored, make our
    ...
    them to file sharing networks. We need to fight for Guerilla Open Access.
